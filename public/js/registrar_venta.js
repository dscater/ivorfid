let btnBuscarCliente = $("#btnBuscarCliente");
$(document).ready(function () {
    var url_obtiene = $('#url_obtiene').val();

    btnBuscarCliente.click(function () {
        if ($("#nit_ci").val().trim() != "") {
            $(".buscando").removeClass("oculto");
            $("#nom_cli").val("")
            $("#fono").val("")
            $.ajax({
                type: "GET",
                url: $("#urlBuscaCliente").val(),
                data: {
                    nit_ci: $("#nit_ci").val()
                },
                dataType: "json",
                success: function (response) {
                    $(".buscando").addClass("oculto");
                    $(".resultado_busqueda").removeClass("oculto");
                    if (response.sw) {
                        $("#nom_cli").val(response.cliente.nom_cli)
                        $("#fono").val(response.cliente.fono)
                        $("#existe_cliente").val(response.cliente.id)
                        $(".cont_res").children("i").removeClass("fa-times text-red");
                        $(".cont_res").children("i").addClass("fa-check text-green");
                        $(".cont_res").children("span").text("Registrado");
                    } else {
                        $("#existe_cliente").val(0)
                        $(".cont_res").children("i").removeClass("fa-check text-green");
                        $(".cont_res").children("i").addClass("fa-times text-red");
                        $(".cont_res").children("span").text("Sin registrar");
                    }
                }
            });
        } else {
            $(".resultado_busqueda").addClass("oculto");
            swal('Error', 'Debe ingresar un nro. de NIT/C.I..', "error");
        }
    });
    ...
    $('#registrar_venta').click(function (e) {
        ...
        let sw_realizar_venta = true;
        if (num_filas > 0) {
            if ($('#nit_ci').val() != '' && $('#nit_ci').val() != null) {
                sw_realizar_venta = true;
            }
            else {
                sw_realizar_venta = false;
                swal('Atención', 'Debe indicar un nro. de NIT/C.I..', "info");
            }
            if ($('#nom_cli').val() != '' && $('#nom_cli').val() != null) {
                sw_realizar_venta = true;
            }
            else {
                sw_realizar_venta = false;
                swal('Atención.', 'Debe indicar el nombre del cliente', "info");
            }

            if (sw_realizar_venta) {
                $(this).text("Cargando...");
...
function realizarVenta() {
    ...
    let data = {
        ...
        'nom_cli': nom_cli,
        'fono': $("#fono").val(),
        'existe_cliente': $("#existe_cliente").val(),
        'nit_ci': nit_ci,
